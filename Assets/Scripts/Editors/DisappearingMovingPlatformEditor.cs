...
[CustomEditor(typeof(DisappearingMovingPlatform)), CanEditMultipleObjects]
public class DisappearingMovingPlatformEditor : Editor
{
    ButtonScript Button;
    public override void OnInspectorGUI()
    {
        base.OnInspectorGUI();
        DisappearingMovingPlatform platform = (DisappearingMovingPlatform)target;

        platform.MoveTime = EditorGUILayout.FloatField("Disappear time", platform.MoveTime);
        EditorUtility.SetDirty(platform);

        if (GUILayout.Button("Add to button", new GUILayoutOption[] { GUILayout.Height(35) }))
        {

            foreach (DisappearingMovingPlatform Platform in targets)
            {
